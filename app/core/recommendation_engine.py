...
from app.models.schemas import MovieRecommendation, PotentialMatch, ScoredMovie
...
class RecommendationEngine:
    ...
    def generate_recommendations(
        ...
        try:
            logger.info(
                f"Generating recommendations in recommendation engine for user {user_id}"
            )
            ...
            if not force_refresh:
                ...
                if cached and len(cached) >= count:
                    logger.info(
                        f"Returning {count} cached recommendations for user {user_id}"
                    )
                    return cached[:count]
            ...
            if not candidate_movies:
                raise RecommendationEngineError(
                    f"No candidate movies found for user {user_id}"
                )

            logger.info(
                f"Found {len(candidate_movies)} candidate movies for user {user_id}"
            )

            # Score all candidates
            scored_movies: List[ScoredMovie] = []
            all_movie_features = commonTasks.get_movies_features(
                list(candidate_movies), db=self.db
            )
            ...
            interacted_movie_features = commonTasks.get_movies_features(
                list(user_profile.get("interactions", {}).keys()), db=self.db
            )
            interaction_lookup_table = defaultdict(dict)

            for movie_features in interacted_movie_features:
                movie_id = movie_features.movie_id
                interaction_score = user_profile.get("interactions", {}).get(
                    movie_id, 0
                )
            ...
            for movie_features in all_movie_features:
                ...
                try:
                    ...
                    logger.error(
                        f"Error scoring movie {movie_id} for user {user_id}: {str(e)}"
                    )
                    continue

            if not scored_movies:
                raise RecommendationEngineError(
                    f"Failed to score any movies for user {user_id}"
                )

            # Sort by score
            scored_movies.sort(key=lambda x: x.score, reverse=True)
            logger.info(
                f"Scored {len(scored_movies)} movies, top score: {scored_movies[0].score:.3f}"
            )
            ...
            potential_matches: List[PotentialMatch] = (
                self.user_matcher.get_potential_matches(
                    user_id,
                    list(map(lambda x: x.movie_id, diverse_movies)),
                    user_profile.get("similar_users", []),
                )
            )
            ...
            for idx, movie_data in enumerate(diverse_movies):
                rec = MovieRecommendation(
                    ...
                    potential_matches=potential_matches[movie_data.movie_id],
                )
            ...
            logger.error(
                f"Unexpected error in generate_recommendations for user {user_id}: {str(e)}"
            )
            raise RecommendationEngineError(
    ...
    def _get_user_profile(self, user_id: int) -> Dict:
        ...
        try:
            ...
            preference_result = self.db.execute(
                preference_query, {"user_id": user_id}
            ).fetchall()
            ...
            rating_count_query = text("""
                ...
                WHERE user_id = :user_id
            """)
            rating_count_result = self.db.execute(
                rating_count_query, {"user_id": user_id}
            ).fetchone()
            if rating_count_result:
                profile["stats"]["total_ratings"] = (
                    rating_count_result.total_ratings or 0
                )
            ...
            preferred_genres_query = text("""
                ...
                WHERE user_id = :user_id
            """)
            preferred_genres_result = self.db.execute(
                preferred_genres_query, {"user_id": user_id}
            ).fetchall()
            ...
            similar_users_query = text("""
                ...
                WHERE user_id = :user_id
            """)
            similar_users_result = self.db.execute(
                similar_users_query, {"user_id": user_id}
            ).fetchall()
            if similar_users_result:
                profile["similar_users"] = [
                    user_id for (user_id,) in similar_users_result
                ]
            ...
            language_preferences_query = text("""
                ...
                WHERE user_id = :user_id
            """)
            language_preferences_result = self.db.execute(
                language_preferences_query, {"user_id": user_id}
            ).fetchall()
            if language_preferences_result:
                profile["language_preferences"] = [
                    language_code for (language_code,) in language_preferences_result
                ]
            ...
            results = self.db.execute(
                interactions_query, {"user_id": user_id}
            ).fetchall()
            ...
            for row in results:
                ...
                if movie_id in profile["interactions"]:
                    profile["interactions"][movie_id] = max(
    ...
    def _get_candidate_movies(self, user_id: int, user_profile: Dict) -> Set[int]:
        ...
        try:
            ...
            is_cold_start = (
                len(user_profile["interactions"]) < 10
                or not user_profile["similar_users"]
            )
            ...
            if user_profile["interactions"]:
                ...
                liked_movies = [
                    movie_id
                    for movie_id, score in user_profile["interactions"].items()
                    if score > 1.0
                ]
            ...
            logger.info(
                f"Total candidates: {len(candidates)} (Cold start: {is_cold_start})"
            )
    ...
    def _get_cached_recommendations(self, user_id: int) -> List[MovieRecommendation]:
        ...
        try:
            ...
            results = self.db.execute(
                cache_query, {"user_id": user_id, "now": datetime.utcnow()}
            ).fetchall()
            ...
            for row in results:
                ...
                potential_matches = [
                    PotentialMatch(
                        user_id=pm[0], match_condition=pm[1], confidence_score=pm[2]
                    )
            ...
            logger.error(
                f"Error getting cached recommendations for user {user_id}: {str(e)}"
            )
            return []
