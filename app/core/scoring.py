...
class SimilarityCalculator:
    ...
    def calculate_movie_similarity(
        movie1_features: MovieFeatures,
        movie2_features: MovieFeatures
    ) -> float:
        ...
        try:
            ...
            # Cast similarity
            cast1 = set(movie1_features.cast_ids)
            cast2 = set(movie2_features.cast_ids)
            if cast1 and cast2:
            ...
            prod1 = set(movie1_features.production_companies)
            prod2 = set(movie2_features.production_companies)
            if prod1 and prod2:
                prod_sim = len(prod1 & prod2) / len(prod1 | prod2)
                similarity_score += weights['production_companies'] * prod_sim
            
            # Language similarity
            if movie1_features.language == movie2_features.language:
                similarity_score += weights['language']
            
            # Year difference (movies from similar era)
            year1 = movie1_features.year or 0
            year2 = movie2_features.year or 0
            year_diff = abs(year1 - year2)
            year_sim = max(0, 1 - (year_diff / 10))  # 10 years = 0 similarity
            similarity_score += weights['year_diff'] * year_sim
            
            # Popularity similarity
            pop1 = movie1_features.popularity or 0
            pop2 = movie2_features.popularity or 0
...
class RecommendationScorer:
    ...
    def score_movie_for_user(
        ...
        try:
            ...
            vote_average = movie_features.vote_average
            vote_count = movie_features.vote_count
            movie_genres = movie_features.genres
            movie_popularity = movie_features.popularity
            ...
            if user_genres and weights.get('genre', 0) > 0:
                genre_score = self._genre_based_score(movie_genres, user_genres)
                if genre_score > 0:
            ...
            if weights.get('popularity', 0) > 0:
                ...
                popularity_float = movie_popularity
                pop_score = min(np.log1p(popularity_float) / 10, 1.0) # TODO - CHANGE 10 TO MAX POPULARITY
            ...
            # Genre match bonus
            if user_genres and len(set(movie_genres) & set(user_genres)) >= 2:
            ...
            # Generate reason
            if not reasons:
                if user_genres and len(set(movie_genres) & set(user_genres)) > 0:
